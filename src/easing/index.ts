...
export const easeInOutExpo = (x: number) => (x === 0
  ? 0
  : x === 1
    ? 1
    : x < 0.5
...
export const easeOutBack = (x: number) => (x === 0
  ? 0 // float-precision
  : 1 + c3 * (x - 1) ** 3 + c1 * (x - 1) ** 2);
export const easeInOutBack = (x: number) => (x < 0.5
  ? ((2 * x) ** 2 * ((c2 + 1) * 2 * x - c2)) / 2
  : ((2 * x - 2) ** 2 * ((c2 + 1) * (x * 2 - 2) + c2) + 2) / 2);
export const easeInElastic = (x: number) => (x === 0
  ? 0
  : x === 1
    ? 1
    : -(2 ** (10 * x - 10)) * sin((x * 10 - 10.75) * c4));
export const easeOutElastic = (x: number) => (x === 0
  ? 0
  : x === 1
    ? 1
